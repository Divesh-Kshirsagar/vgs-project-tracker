...
export const generateTasks = (): Task[] => {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const startDate = Math.random() < 0.5 ? new Date(today.getTime() - Math.floor(Math.random() * 10) * 24 * 60 * 60 * 1000).toISOString() : null; 
    const dueDate = new Date(today.getTime() + Math.floor(Math.random() * 20) * 24 * 60 * 60 * 1000).toISOString(); 
