...
export const TimelineView: React.FC<TimelineViewProps> = ({ tasks }) => {
  const { year, month, daysInMonth, currentDay, daysArray } = useMemo(
    () => getCurrentMonthData(),
    [],
  );
        ...
        >
          ...
          {visibleTasks.map((task) => {
            ...
            if (task.startDate) {
              ...
              startDay =
                startMonth < month || startDateObj.getFullYear() < year
                  ? 1
                  : startDateObj.getDate();
              endDay =
                dueMonth > month || dueDateObj.getFullYear() > year
                  ? daysInMonth
                  : dueMonthDay;
            } else {
            ...
            return (
              <React.Fragment key={task.id}>
                ...
                >
                  ...
                  >
                    {isSingleDay && (
                      <span className="w-2 h-2 bg-white rounded-full opacity-75 mr-1" />
                    )}
                  </div>
                </div>
              </React.Fragment>
