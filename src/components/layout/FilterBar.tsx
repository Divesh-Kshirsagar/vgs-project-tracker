...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, updateFilters, clearFilters }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 w-full bg-gray-50 p-3 rounded-lg border">
      <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider mr-2">Filters:</span>
      ...
      {hasActiveFilters && (
        <Button variant="ghost" onClick={clearFilters} className="ml-auto text-xs py-1 px-2 h-auto text-red-600 hover:text-red-700 hover:bg-red-50">
          Clear filters
        </Button>
      )}
    </div>
